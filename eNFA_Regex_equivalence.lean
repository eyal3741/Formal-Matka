...
lemma if_0mod2_step_is_0mod2 (A' : εNFA alphabet ℕ) (hA': A'.is_0mod2_εNFA) (q₁ q₂ : ℕ) (σ : Option alphabet) :
...
lemma if_1mod2_step_is_1mod2 (A' : εNFA alphabet ℕ) (hA': A'.is_1mod2_εNFA) (q₁ q₂ : ℕ) (σ : Option alphabet) :
    (q₂ ∈ A'.step q₁ σ) → (q₁ % 2 = q₂ % 2) := by
    intro hmem
    obtain ⟨ A, hA ⟩ := hA'
    subst hA
...
lemma if_0mod2_path_is_0mod2 (A' : εNFA alphabet ℕ) (hA': A'.is_0mod2_εNFA) (q₁ q₂ : ℕ) (x : List (Option alphabet)) :
...
lemma if_1mod2_path_is_1mod2 (A' : εNFA alphabet ℕ) (hA': A'.is_1mod2_εNFA) (q₁ q₂ : ℕ) (x : List (Option alphabet)) :
    (A'.IsPath q₁ q₂ x) → (q₁ % 2 = q₂ % 2) := by
    intro hpath
    have ⟨ A, hA ⟩ := hA'
    subst hA
...
def εNFA_comp (A₁: εNFA alphabet ℕ) (A₂: εNFA alphabet ℕ) (hA₁ : A₁.is_0mod2_εNFA) (hA₂ : A₂.is_1mod2_εNFA) : εNFA alphabet ℕ := {
    start  := A₁.start
    accept := A₂.accept
    step   := fun q c =>
        if q % 2 = 0 then
...
lemma comp_step_1mod2_to_1mod2 (A A₁' A₂': εNFA alphabet ℕ)
    (hA₁': A₁'.is_0mod2_εNFA) (hA₂': A₂'.is_1mod2_εNFA) (hA: A = εNFA_comp A₁' A₂' hA₁' hA₂') (q₁ q : ℕ)  :
...
lemma comp_path_0mod2 (A A₁' A₂': εNFA alphabet ℕ)
    (hA₁': A₁'.is_0mod2_εNFA) (hA₂': A₂'.is_1mod2_εNFA) (hA: A = εNFA_comp A₁' A₂' hA₁' hA₂')
...
lemma comp_word_decomposition (A A₁' A₂': εNFA alphabet ℕ)
    (hA₁': A₁'.is_0mod2_εNFA) (hA₂': A₂'.is_1mod2_εNFA) (hA: A = εNFA_comp A₁' A₂' hA₁' hA₂') (x : List alphabet) :
...
lemma kstar_decompose_path
    (A A' : εNFA alphabet ℕ) (hA': A'.is_0mod2_εNFA) (hA : A = εNFA_kstar A')
    (qs qf : ℕ) (x : List (Option alphabet)) :
    A.IsPath qs qf x → x = [] ∨ ∃ (qs' qf' : ℕ), ∃ a b : List alphabet,
    (x = a ++ b ∧ A.IsPath qs qs' [] ∧ A'.IsPath qs' qf' a ∧ A.IsPath qf' qf b) := by
    intro h_x_path_in_A
    by_cases h_qs_eq_qf: x = []
    case pos => left; exact h_qs_eq_qf
    case neg =>
        right
        cases h_x_path_in_A
        case nil => sorry
...
lemma kstar_accepts_decomposed_nonempty
    (A A' : εNFA alphabet ℕ) (hA': A'.is_0mod2_εNFA) (hA : A = εNFA_kstar A') (x : List alphabet) :
        ...
        | cons t qs qf c tail h_step h_rest ih =>
            ...
            case pos =>
                ...
                | some σ =>
                    ...
                    have h_x_σ : x = [σ] := by
                        exact Multiset.coe_eq_singleton.mp (congrArg Multiset.ofList h_x_σ_nil)
                    ...
                    case left =>
                        ...
                        have : some σ ≠ none := by exact Option.some_ne_none σ
...
lemma Star_Regex_to_εNFA (r : RegularExpression alphabet) :
    ...
    case mp =>
        ...
        case cons head tail h_induction =>
            ...
            have h_head_acc : head ∈ A.accepts := by
                ...
                obtain ⟨qs1, qf1, headOpt, h_qs1, h_qf1, h_headOpt, h_path⟩ := h_head
                refine ⟨1, 1, ([none] ++ headOpt ++ [none]), ?_, ?_, ?_, ?_⟩
                · simp [A, εNFA_kstar]
                · simp [A, εNFA_kstar]
                · simpa [List.reduceOption_append, h_headOpt]
                ·
                   have h1_not_acc : (1 : ℕ) ∉ A'.accept := by
                    simp [A', to_0mod2_εNFA]
                   have h_step1 : qs1 ∈ A.step 1 none := by
                     simp [A, εNFA_kstar, h1_not_acc, h_qs1]
                   have h_eps1 : A.IsPath 1 qs1 [none] :=
                        A.isPath_singleton.mpr h_step1
                   have h_mid : A.IsPath qs1 qf1 headOpt :=
                        path_if_contains A A' h_contains qs1 qf1 headOpt h_path
                   have h_step2 : (1 : ℕ) ∈ A.step qf1 none := by
                        simp [A, εNFA_kstar, h_qf1]
                   have h_eps2 : A.IsPath qf1 1 [none] :=
                        A.isPath_singleton.mpr h_step2
                   have h_mid' : A.IsPath qs1 1 (headOpt ++ [none]) :=
                        A.isPath_append.mpr ⟨qf1, h_mid, h_eps2⟩
                   have h_1_to_qf1 : A.IsPath 1 qf1 ([none] ++ headOpt) :=
                         A.isPath_append.mpr ⟨qs1, h_eps1, h_mid⟩
                   have h_full : A.IsPath 1 1 (([none] ++ headOpt) ++ [none]) :=
                         A.isPath_append.mpr ⟨qf1, h_1_to_qf1, h_eps2⟩

                   simpa [List.append_assoc] using h_full
    ...
    case mpr =>
        ...
        have hx :
            ...
            kstar_word_decomposition (A := A) (A' := A') (hA := rfl) (x := x) h_in_A
