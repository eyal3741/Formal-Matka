...
def to_trim (A : εNFA α ℕ) (i j k : ℕ) : εNFA α ℕ := {
    start  := { i }
    accept := { j }
    step   := fun q c =>
        if j > k then
...
def is_trim (A' : εNFA α ℕ) (i' j' k' : ℕ) :=
    ∃ (A : εNFA α ℕ), A' = to_trim A i' j' k'
...
lemma singular_finite_accepts_iff_trim_accepts (A A': εNFA α ℕ) (s a n: ℕ)
...
lemma regex_for_path_contains_ij (A : εNFA α ℕ) (i j k k' : ℕ) (h_k' : k' ≤ k) :
...
lemma regex_for_path_contains_ik (A : εNFA α ℕ) (i j k k' : ℕ) (h_k' : k' ≤ k) :
...
lemma regex_for_path_contains_kk (A : εNFA α ℕ) (i j k k' : ℕ) (h_k' : k' ≤ k) :
...
lemma regex_for_path_kk_star_is_path_in_kk (A : εNFA α ℕ) (k : ℕ) (L': List (List (Option α))) :
    (∀ (x' : List (Option α)), (x' ∈ L' → Nonempty (A.Path k k x'))) → Nonempty (A.Path k k L'.flatten) := by
    intro h_y
    ...
    case cons head tail h_induction =>
        have h_tail : (∀ y' ∈ tail, Nonempty (A.Path k k y')) := by
            simp at h_y
            exact h_y.right
...
lemma regex_for_path_contains_kj (A : εNFA α ℕ) (i j k k' : ℕ) (h_k' : k' ≤ k) :
...
lemma split_path_by_state (A : εNFA α ℕ) (i j k : ℕ) (x': List (Option α)) (path_ij: A.Path i j x'):
    (k ∈ path_ij.supp) → ∃ (steps_ik steps_kk steps_kj : List (ℕ × Option α × Prop)),
        steps_ik ++ steps_kk ++ steps_kj = path_ij.steps ∧
        (k ∉ (Path.steps.states steps_ik).tail) ∧ (k ∉ (Path.steps.states steps_kj).tail) := by
    intro h_k_supp
    induction path_ij
    case nil s =>
        use [(s, none, True)], [], []
        simp [Path.steps.states]
    case cons t s u c tail h_step path h_induction =>
        by_cases k = s
        case pos h_sk =>
            subst h_sk
            use [(k, none, True)]
            by_cases k ∈ path.supp
            case pos h_k_in_path =>
                obtain ⟨ steps_ik, steps_kk, steps_kj, h_path, h ⟩ := h_induction h_k_in_path
                use steps_ik ++ steps_kk, steps_kj
                simp
            case neg => sorry
        case neg => sorry
